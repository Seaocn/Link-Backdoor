...
from utils import load_data, mask_test_edges, preprocess_graph, get_roc_score,sparse_mx_to_torch_sparse_tensor_GNAE,GBA_LOSS,get_AMC_score
import os
...
parser.add_argument('--num_clusters',type=int,default=6,help='the number of class')
...
def find_gard_max(grad,features,dataset,backnode0,backnode1):
    ...
    if dataset=="pubmed" :
        for i in range(feat_dim):
            features[backnode0- 1][i] = features[backnode0- 1][i]-step if grad_array[backnode0- 1][i]>0 else features[backnode0- 1][i]+step
            features[backnode1 - 1][i] = features[backnode1 - 1][i]-step if grad_array[backnode1 - 1][i]>0 else features[backnode1 - 1][i]+step

            # features[node - 2][i] = 0  if grad_array[node - 2][i] > 0 else 1
            # features[node - 1][i] = 0 if grad_array[node - 1][i] > 0 else 1
            if features[backnode0- 1][i] >1 :
                features[backnode0- 1][i]=1
            if features[backnode0- 1][i] <0:
                features[backnode0- 1][i] = 0
            if features[backnode1 - 1][i] >1 :
                features[backnode1 - 1][i]=1
            if features[backnode1 - 1][i] <0:
                features[backnode1 - 1][i] = 0
    else:
        for i in range(feat_dim):
            features[backnode0- 1][i] = 1 if grad_array[backnode0- 1][i] < 0 else 0
            features[backnode1 - 1][i] = 1 if grad_array[backnode1 - 1][i] < 0 else 0
    return  features
...
def random_nodes(adj,features,targetnode):
    n_nodes, feat_dim = features.shape
    index_1 = np.random.randint(0, n_nodes)
    index_2 = np.random.randint(0,n_nodes)
    while index_1 in targetnode:
        index_1 = np.random.randint(0, n_nodes)
    while index_2 in targetnode:
        index_2 = np.random.randint(0, n_nodes)
    while index_1==index_2:
        index_2 = np.random.randint(0, n_nodes)
    adj=adj.A
    for i in range(n_nodes):
        adj[i][index_1-1]=adj[index_1-1][i]=0
        adj[i][index_2 - 1] = adj[index_2 - 1][i] = 0
    adj = sp.csr_matrix(adj)

    return index_1,index_2,adj
...
def inject_lables_add2_node(adj_t, trigger_node_list, n_b_e, subg,back_node_0,back_node_1):#加入两个节点的攻击方法的测试集
    for i in range(n_b_e):
        jd = trigger_node_list[i]
        adj_t[back_node_0 - 1][jd[0]] = adj_t[jd[0]][back_node_0 - 1] = 1
        adj_t[back_node_0 - 1][jd[1]] = adj_t[jd[1]][back_node_0 - 1] = 1  # 与新节点相连
        adj_t[back_node_1 - 1][jd[0]] = adj_t[jd[0]][back_node_1 - 1] = 1
        adj_t[back_node_1 - 1][jd[1]] = adj_t[jd[1]][back_node_1 - 1] = 1
        adj_t[jd[0]][jd[1]] = adj_t[jd[1]][jd[0]] = 1 # 相当于修改类标
    adj_t[back_node_1 - 1][back_node_0 - 1] = adj_t[back_node_0 - 1][back_node_1 - 1] = 1
    print("Injection train finished")

def inject_edge2_node(adj_t, trigger_node_list, n_b_e, subg,back_node_0,back_node_1):#加入两个节点的攻击方法的测试集
    for i in range(n_b_e):
        jd = trigger_node_list[i]
        adj_t[back_node_0-1][jd[0]] = adj_t[jd[0]][back_node_0-1] = 1
        adj_t[back_node_0-1][jd[1]] = adj_t[jd[1]][back_node_0-1] = 1#与新节点相连
...
def gae_for(args):
    ...
    back_node_0, back_node_1,adj = random_nodes(adj_train,features,target_nodes)
    ...
    inject_edge2_node(adj_tr, trigger_node_lists_train, num_back_edge_train, sub_g,back_node_0,back_node_1)
    adj_train = sp.csr_matrix(adj_tr)
    adj = adj_train


    adj_tr_lable=adj_tr
    inject_lables_add2_node(adj_tr_lable, trigger_node_lists_train, num_back_edge_train, sub_g,back_node_0,back_node_1)
    ...
    inject_edge2_node(adj_tr, trigger_node_lists_test, num_back_edge_test, sub_g,back_node_0,back_node_1)
    ...
    for epoch in range(args.epochs):
        if epoch< 80:
            ...
            '''---------------------------------------GNAE--------------------------------------'''
            # model_GNAE.train()
            # optimizer_GNAE.zero_grad()
            # recovered,mu = model_GNAE(features,adj_norm_GNAE)
            ...
            '''------------------------------------VGNAE---------------------------------------------'''
            # model_VGNAE.train()
            # optimizer_VGNAE.zero_grad()
            # recovered,mu,logvar = model_VGNAE(features,adj_norm_GNAE)
            # loss = loss_function(preds=recovered, labels=adj_label_orig,
            #                     mu=mu, logvar=logvar, n_nodes=n_nodes,
            #                     norm=norm, pos_weight=pos_weight)
            # loss.backward()
            # optimizer_VGNAE.step()
            #
            # hidden_emb = mu.data.numpy()
            # roc_curr, ap_curr, _, _ = get_roc_score(hidden_emb, adj_orig, val_edges, val_edges_false)
            # cur_loss=loss
            ...
            if (epoch==80):
                ...
                # features_array = features.numpy()
                # features_array[n_nodes - 1] = pop[0][0]
                # features_array[n_nodes - 2] = pop[0][1]
                ...
                # recovered, mu = model_GNAE(features_var, adj_norm_train_GNAE)
                # recovered, mu, logvar = model_VGNAE(features_var, adj_norm_train_GNAE)
                ...
                loss_grad= GBA_LOSS (recovered,trigger_node_lists,args.attalink)
                gard=torch.autograd.grad(loss_grad,features_var,retain_graph=True)[0].data
                features=find_gard_max(gard,features,args.dataset_str,back_node_0,back_node_1)

                # heart=0
                # features_var = torch.autograd.Variable(features, requires_grad=True)
                # # recovered, mu, logvar = model(features_var, pso_adj_norm)
                # recovered, mu = model_GNAE(features_var, adj_norm_train_GNAE)
                # loss_grad = GBA_LOSS(recovered, trigger_node_lists, 705)
                # heart=0
                '''-----------------------------------------------------------------'''
                ...
                # features_array = features.numpy()
                #
                # features_array[n_nodes - 1] = np.where(gbestpop[0] > 0.5, 1,0)
                # features_array[n_nodes - 2] = np.where(gbestpop[1] >0.5 , 1,0)
                # features = torch.from_numpy(features_array)
            ...
            # model_GNAE.train()
            # optimizer_GNAE.zero_grad()
            # recovered, mu = model_GNAE(features, adj_norm_train_GNAE)
            ...
            # model_VGNAE.train()
            # optimizer_VGNAE.zero_grad()
            # recovered, mu, logvar = model_VGNAE(features, adj_norm_train_GNAE)
            # loss = loss_function(preds=recovered, labels=adj_label,
            #                      mu=mu, logvar=logvar, n_nodes=n_nodes,
            #                      norm=norm, pos_weight=pos_weight)
            # loss.backward()

...
from utils import load_data, mask_test_edges, preprocess_graph, get_roc_score,sparse_mx_to_torch_sparse_tensor_GNAE,GBA_LOSS,find_gard_max,get_AMC_score,find_subgraph
import os
...
parser.add_argument('--pre_epochs', type=int, default=1, help='Number of epochs to train.')
...
parser.add_argument('--num_clusters',type=int,default=3,help='the number of class')
...
parser.add_argument('--attalink',type=int,default=7,help='540,470,4430')
parser.add_argument('--alllink',type=int,default=8,help='876,960,8305')
...
def inject_edge2_node(adj_t, trigger_node_list, n_b_e, subg):#加入两个节点的攻击方法的测试集
    for i in range(n_b_e):
        ...
        adj_t[adj_t.shape[0] - 1][jd[0]] = adj_t[jd[0]][adj_t.shape[0] - 1] = 1
        adj_t[adj_t.shape[0] - 1][jd[1]] = adj_t[jd[1]][adj_t.shape[0] - 1] = 1
        # adj_t[adj_t.shape[0] - 3][jd[0]] = adj_t[jd[0]][adj_t.shape[0] - 3] = 1
        # adj_t[adj_t.shape[0] - 3][jd[1]] = adj_t[jd[1]][adj_t.shape[0] - 3] = 1
        # adj_t[jd[0]][jd[1]] = adj_t[jd[1]][jd[0]] = 1  # 相当于修改类标
    adj_t[adj_t.shape[0]-1][adj_t.shape[0]-2]=adj_t[adj_t.shape[0]-2][adj_t.shape[0]-1]=1
    print("Injection test finished")
...
def gae_for(args):
    ...
    adj, features ,_= add_node(adj, features)  # 增加一个新的节点
    adj, features,_ = add_node(adj, features)
    ...
    adj = adj_train
    find_subgraph(adj,back_nodes_0[0],back_nodes_1[0],n_nodes,args.dataset_str,3)
    ...
    hidden_emb = None
    for epoch in range(args.epochs+args.pre_epochs):
        if epoch< args.pre_epochs:
            ...
            if (epoch==args.pre_epochs)and(epoch<160):
    ...
    # print("train_asr=", "{:.5f}".format(tr_asr),"train_asr=", "{:.5f}".format(AMC),"opt_t=","{:.5f}".format(op_t))

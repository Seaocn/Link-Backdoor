...
parser.add_argument('--attalink',type=int,default=470,help='540,470,4430')
parser.add_argument('--alllink',type=int,default=960,help='876,960,8305')
...
def mask_attack_edge(adj,list):
    adj=adj.A
    z=int(len(list)/2)
    for i in range (z):
        adj[int(list[i*2])][int(list[i*2+1])]=adj[int(list[i*2+1])][int(list[i*2])]=0
    adj=sp.csr_matrix(adj)
    return adj
...
def gae_for(args):
    ...
    n_nodes, feat_dim = features.shape
    # adj_fusion=copy.deepcopy(adj)
    # adj=mask_attack_edge(adj_fusion,target_nodes)
    ...
    for epoch in range(args.epochs):
        if epoch< 80:
            ...
            if (epoch%20==0)and(epoch<160):
                ...
                # recovered, mu, logvar = model(features_var, adj_norm_train)
                # # recovered, mu = model_GNAE(features_var, adj_norm_train_GNAE)
                # # mbeds, _, _, S = model_GIC.embed(features_var, adj_norm_train, None, args.beta)
                # # embeds = embeds / embeds.norm(dim=1)[:, None]
                # # recovered = torch.mm(embeds, embeds.t())
                # loss_grad = GBA_LOSS(recovered, pso_trigger_node_lists_train, args.attalink)
                # print('trigger loss2', loss_grad.item())
                # heart=0
                '''-----------------------------------------------------------------'''
